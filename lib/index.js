...
const replace = (...args) => val => typeof val === 'string' ? val.replace(...args) : val;

const replaceTabsAndSpaces = replace(/\s\s+/g, ' ');
const makeAccessId = compose(replaceTabsAndSpaces, trim);
const makeTestId = compose(replace(/\W/g, ''), replace(/\s/g, '_'), toLowerCase, makeAccessId);

const checkForAttribute = (name, attributes) => attributes.some(attr => attr && typeof attr.name === 'object' && attr.name.name === name);
...
module.exports = function (_ref, opts) {
  const t = _ref.types;
  const nameComps = Array.isArray(opts.components) && opts.components.length ? opts.components : defaultComponents;
  return {
    visitor: {
      JSXElement: {
        enter(path) {
          try {
            ...
            if (nameComps.indexOf(nameComp) !== -1) {
              const {
                children
              } = path.node; // remove empty children
              ...
              if (!len) {
              ...
              } else if (cookedChildren.every(({
                type
              }) => type === 'JSXText')) {
                const joinedStringLiteral = cookedChildren.reduce((acc, child, index) => {
                  const {
                    value: v
                  } = child;
                ...
                if (!reducedChildren.length) {
                  ...
                  const expressions = reducedChildren.reduce((acc, child, idx) => {
                    if (child && child.type === 'JSXExpressionContainer') {
                      if (idx === reducedChildren.length - 1) {
                        possibleTailQuasis.push(t.templateElement({
                          raw: '',
                          cooked: ''
                        }, true));
                      }
                  ...
                  const possibleEmptyFront = rawQuasis.length === expressions.length ? [t.templateElement({
                    raw: '',
                    cooked: ''
                  }, false)] : [];
                  const quasis = possibleTailQuasis.length ? [...rawQuasis, ...possibleTailQuasis] : [...possibleEmptyFront, ...rawQuasis];
                  const tmpltLiteral = t.templateLiteral(quasis, expressions);
                  attributeValue = t.JSXExpressionContainer(tmpltLiteral);
                }
              }

              const hasTestId = checkForAttribute('testID', path.node.openingElement.attributes);
              const hasAccessibilityLabel = checkForAttribute('accessibilityLabel', path.node.openingElement.attributes);

              if (Array.isArray(path.node.openingElement.attributes)) {
                if (!hasTestId) {
                  const testIdProp = t.jSXAttribute(t.jSXIdentifier('testID'), attributeValue);
                  path.node.openingElement.attributes.push(testIdProp);
                }

                if (!hasAccessibilityLabel) {
                  const accessibilityLabel = t.jSXAttribute(t.jSXIdentifier('accessibilityLabel'), attributeValue);
